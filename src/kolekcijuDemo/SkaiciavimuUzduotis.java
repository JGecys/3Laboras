...
public class SkaiciavimuUzduotis {
     ...
     * @param lev - nurodomas hierarchijos lygis
     * @param dir - nurodoma einamojo lygio direktorija
     */
    public void rastiVisusFailus(int lev, File dir) {
        for (File f : dir.listFiles()) 
            if (f.isFile()) 
                visiFailai.add(new Failas(f.getAbsolutePath(), f.length(), lev));
            else 
                rastiVisusFailus(lev + 1, f);
    }
    /**************************************************************************
     * Metodas skirtas suskiačiuoti kiek yra skirtingų failų tipų
     */
    ...
    public void skaičiuotiAtskirusGamintojus() {
       // panaudojant Map struktūrą, randa skirtingus gamintojus
        Map<String, SumosGamintojo> gamintojai =
                new HashMap<String, SumosGamintojo>();
...
class SumosGamintojo {
   ...
   public String toString(){
    ...
    public static void main(String[] args) {
      ...
      SkaiciavimuUzduotis uzd = new SkaiciavimuUzduotis();       // 1-a dalis
